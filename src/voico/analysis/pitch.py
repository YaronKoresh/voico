...
class PitchAnalyzer:
    ...
    def _compute_hnr(self, audio: np.ndarray, f0_mean: float) -> float:
    ...
    def _autocorrelation_detect(
        ...
        default_fft_size = self._next_power_of_two(2 * window_size)
        for t in range(n_frames):
            ...
            fft_size = (
                ...
                else self._next_power_of_two(2 * len(frame))
            )
            ...
            energy_left = frame_sq_cumsum[np.clip(n - taus - 1, 0, n - 1)]
            energy_right = frame_sq_cumsum[n - 1] - frame_sq_cumsum[taus - 1]
            difference[1:] = np.maximum(
                energy_left + energy_right - 2 * acf[1 : frame_max_lag + 1], 0.0
            )
            cumsum_diff = np.cumsum(difference[1:])
            normalized_difference = np.ones(frame_max_lag + 1)
            tau_values = np.arange(1, frame_max_lag + 1, dtype=np.float64)
